...
function App() {
  ...
  const cvSteps = [
    ...
    "Millorant contrast...",
    "Eliminant soroll...",
    "Detectant línies de text...",
    "Detectant paraules...",
    "Segmentant lletres...",
    "Calculant variables gràfiques...",
  ...
  const setupFile = (selectedFile) => {
    ...
    // Create preview
    const ext = selectedFile.name.split('.').endLine;
  ...
  const startAnalysis = async () => {
    ...
    // Animate CV processing steps (total 9 steps, takes around 4 seconds to reach step 8, then waits for backend response)
  ...
  const handlePDFExport = async () => {
    ...
    try {
      ...
      a.href = url;
      a.download = `GraphoAI_Report_${analysisResult.filename.split('.')[0]}.pdf`;
  ...
  const handleJSONExport = () => {
    ...
    const a = document.createElement('a');
    a.href = jsonString;
    a.download = `GraphoAI_Analysis_${analysisResult.filename.split('.')[0]}.json`;
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
  };
  ...
  return (
    <div className="min-h-screen flex flex-col font-sans transition-colors duration-300 dark:bg-[#080c14]">
      {/* Navbar Banner */}
      <header className="sticky top-0 z-50 glass-panel border-b border-slate-200 dark:border-slate-800 px-6 py-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold font-heading text-slate-900 dark:text-white leading-tight m-0 select-none">
              GraphoAI
            </h1>
            <p className="text-[11px] text-slate-500 dark:text-slate-400 font-medium tracking-wide m-0">
      ...
      <main className="flex-1 w-full max-w-7xl mx-auto px-4 py-8 md:py-12">
        ...
        {!analysisResult && !isAnalyzing && (
          <div className="max-w-2xl mx-auto flex flex-col items-center justify-center mt-6">
            ...
            <div 
              ...
              className={`w-full p-10 md:p-14 border-2 border-dashed rounded-2xl cursor-pointer flex flex-col items-center justify-center transition-all duration-300
                ${dragActive 
            ...
            >
              ...
              <div className="w-16 h-16 rounded-full bg-blue-50 dark:bg-blue-950/30 flex items-center justify-center mb-4 text-blue-600 dark:text-blue-400">
              ...
              <p className="text-xs text-slate-400 dark:text-slate-500 text-center max-w-xs leading-normal mb-3">
            ...
            {/* Uploaded File Panel */}
            {file && (
              <div className="w-full mt-6 p-4 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl flex items-center justify-between shadow-sm animate-fadeIn">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-5 rounded-2xl shadow-sm">
              <div>
                ...
                >
                  ...
                  Analitzar un altre document
                </button>
                <h2 className="text-2xl font-bold font-heading text-slate-950 dark:text-white flex items-center gap-2">
                  Resultats de l'Anàlisi Grafopsicològica
                ...
                <p className="text-xs text-slate-400 dark:text-slate-500 mt-1">
                  Fitxer: <span className="font-semibold text-slate-600 dark:text-slate-400">{analysisResult.filename}</span>
                </p>
              </div>
                    ...
                    {activeTab === 'laboral' && (
                      <div className="space-y-4 animate-fadeIn">
                        <p className="text-xs text-slate-400 dark:text-slate-500">
                          Aquesta persona podria sentir-se més còmode o motivada treballant en els següents entorns:
                        </p>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    {[
                      ...
                      <div key={imgStep.key} className="border border-slate-100 dark:border-slate-800 rounded-xl overflow-hidden bg-slate-50 dark:bg-slate-950/20">
                        <div className="aspect-[3/4] overflow-hidden">
                          <img 
                            src={analysisResult.visualizations[imgStep.key]} 
                            alt={imgStep.name} 
                            className="w-full h-full object-contain hover:scale-105 transition-transform duration-300 cursor-zoom-in"
                            onClick={() => {
                              const newWindow = window.open();
                              newWindow.document.write(`<img src="${analysisResult.visualizations[imgStep.key]}" style="max-width:100%; max-height:100vh; display:block; margin:auto;" />`);
                            }}
                          />
                        </div>
